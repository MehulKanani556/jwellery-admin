...
export default function ReturnOrder() {
    ...
    // serch
    const filteredData = returnOrders.filter(data =>
        data.return_status === "pending" &&
        (data.product && data.product.toString().toLowerCase().includes(searchValue.toLowerCase()) ||
        data.customer && data.customer.toString().toLowerCase().includes(searchValue.toLowerCase()))
    );
    useEffect(() => {
        setFilterReturn(filteredData);
    }, [searchValue]);
